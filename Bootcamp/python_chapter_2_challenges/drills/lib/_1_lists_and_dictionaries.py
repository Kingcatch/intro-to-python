...
def we_joined_the_beatles(my_list):
    the_beatles = ['john', 'paul', 'george', 'ringo'] 
    return the_beatles + my_list
...
def remove_nones_from_list(my_list):
    return [item for item in my_list if item is not None]
...
def all_keys(one_dict):
    return list(one_dict.keys())
...
def remove_nones_from_dictionary(one_dict):
    return {key: value for key, value in one_dict.items() if value is not None}
